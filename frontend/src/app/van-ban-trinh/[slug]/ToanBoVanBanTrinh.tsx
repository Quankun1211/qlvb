...
const allStatuses = [
  ...
  "Chờ trình LĐ bộ", "Chờ thư ký Bộ tiếp nhận", "Chờ LĐ Bộ ký duyệt", 
  "LĐ Bộ đã ký duyệt", "Chờ chuyển VT Bộ", "Đã phát hành", "Trưởng phòng trả về", 
  "LĐ đơn vị trả về", "VT đơn vị trả về", "Thư ký Bộ trả về", "LĐ Bộ trả về", 
  "Bộ trưởng trả về", "VT Bộ trả về", "Đã chuyển VT Vụ Tổ chức", 
  "Đang xin ý kiến lãnh đạo Bộ", "LĐB đã ký và Trình xin ý kiến", 
  "Đang trình LĐ Bộ", "Đã được làm lại", "Đã tạm dừng"
];
...
export default function ToanBoVanBanTrinh() {
    ...
    <div className="w-full min-h-full bg-white shadow-sm border border-gray-200">
      <div className="p-4 border-b border-gray-200">
        <div className="flex justify-between items-start mb-4">
          <h1 className="text-[22px] font-normal text-gray-800">Danh sách văn bản trình tham gia</h1>
              ...
              paginatedData.map((row, index) => (
                <tr key={index} className="border-b border-gray-200 hover:bg-gray-50 transition-colors text-gray-800">
                  ...
                  <td className="p-3 text-center">
                    <span className="inline-block px-3 py-1 bg-gray-500 text-white rounded-full text-[11px] font-semibold">{row.trangThai}</span>
                  </td>
                </tr>
              ))
            ) : (
